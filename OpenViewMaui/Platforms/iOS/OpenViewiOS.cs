...
{
    internal class OpenViewiOS : IOpenViewPlatform
    {
        ...
        public Task AddAsync(OpenViewPage page)
        {
            ...
            if (IsiOS13OrNewer)
            {
                ...
                if (connectedScene != null && connectedScene is UIWindowScene windowScene)
                    window = new OpenViewWindow(windowScene);
                else
                    window = new OpenViewWindow();
            ...
            window.RootViewController = new OpenViewPageRenderer(handler);
        ...
        public async Task RemoveAsync(OpenViewPage page)
        {
            if (page == null)
                throw new Exception("Popup page is null");

            var handler = page.Handler as OpenViewPageHandler;
